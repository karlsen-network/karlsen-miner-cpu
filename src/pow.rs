...
impl State {
    ...
    pub fn new(id: usize, block: RpcBlock) -> Result<Self, Error> {
        ...
        let hasher = PowB3Hash::new(pre_pow_hash, header.timestamp as u64);

        Ok(Self { id, nonce: 0, target, block, hasher, header_version })
    }
    ...
    pub fn calculate_khashv2(&self, context: &mut FishHashContext) -> Uint256 {
        ...
        // println!("hash-3 : {:?}", hash);
        Uint256::from_le_bytes(hash.to_le_bytes())
    }
